...
def extract_implication(text):
    ''' Sei que é má prática MAAAAAAAAAAS acredito que dê para resolver isso(e possivelmente os proximos operadores também) usando splits ao invés de regex
        Desde já, peço seu perdão
    '''
    result = [text]
    matches = text.split(">",1)
    if(len(matches) > 1):
        result.extend(extract_implication(matches[1]))
       
    return result
text = " ((a.b)+c) (a.~(b>c)+d) ((~a+(b.(c+d))>e)>f) x.y>z i>j>k"
text_implication = "i>j>k>l"
all_matches = extract_implication(text_implication)
